        ...
        {#each socials as social}
          <li>
            <a href={social.targetUrl}>
              <img alt={social.iconAlt} src={social.iconSrc} class="size-5 pointer-coarse:size-6 opacity-75 invert" />
            </a>
          </li>
        {/each}
      </ul>
...
<footer class="flex flex-row justify-between text-fg-tertiary dark:text-fg-tertiary-dark text-xs">
  ...
  <p class="mx-(--spc-std)">Come here from one of my friends' sites? Use these buttons to navigate the webring.</p>
